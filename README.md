...
### 2.3	Función main
El programa revisará si el usuario le pasa como parámetro el número de slaves a utilizar. En caso que el usuario no pase ningún parámetro extra, el programa se ejecutará con un número determinado de slaves, 10. Por otro lado, si el usuario pasa un valor, se comprobará que sea un valor válido (1 ≤ N_SLAVES ≤ 100) y cambiará el número de slaves para la ejecución. Si éste no es válido, el programa se ejecutará con el valor por defecto. 
Cuantos más slaves se estén ejecutando concurrentemente en el programa, más llamadas simultaneas al COS estarán haciendo. Cuantas más consultas se ejecuten por unidad de tiempo, más posible es que el programa colapse al no poder procesarlas. Es por ello que hemos optado por ajustar el tiempo entre las llamadas en función del número de slaves siguiendo la fórmula siguiente: 
<p align="center">timeSleep = 1 + t · N_SLAVES</p> 
<p align="center">dónde t es el tiempo de espera de cada uno de los slaves.</p> 

El programa principal ejecutara la llamada tanto a la función de master como a la de los slaves y esperará a que acaben. Una vez haya acabado esta ejecución, se procederá a comprobar el correcto funcionamiento de la exclusión mutua comparando la lista resultado obtenida de la ejecución del master con la lista que se encuentra almacenada en el fichero “result.txt” del COS. En caso que las dos listas se correspondan, significara que se ha garantizado la exclusión mutua mientras que si las dos listas no se corresponden, habrá habido algún error a la hora de conceder los permisos de escritura. Se puede ejecutar este programa de dos formas distintas, en función de si quieremos cambiar el número de slaves que utilizamos o si queremos el número por defecto. 
